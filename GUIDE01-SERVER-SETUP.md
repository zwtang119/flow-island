...
**1.3 启动服务器**
1.  在终端中，根据您的操作系统，使用相应命令启动服务器。**将 `YOUR_GEMINI_API_KEY` 替换为您自己的 Gemini API 密钥。**
    ...
    *   **Windows (CMD)**:
        ```cmd
        set API_KEY=YOUR_GEMINI_API_KEY&&node server.js
        ```
    
    *   **Windows (PowerShell)**:
        ```powershell
        $env:API_KEY="YOUR_GEMINI_API_KEY"; node server.js
        ```
...
---
**部署已完成第一部分。** 下一步，请继续进行 **[第二部分：Watchy 固件刷写](./GUIDE02-FIRMWARE-FLASHING.md)**。